...
Grid.mixin({
	...
	bgEventSegEl: function(seg, el) {
		return this.salarySheetView.resolveEventEl(seg.event, el); // will filter through eventRender
	},
	...
	triggerSegMouseover: function(seg, ev) {
		if (!this.mousedOverSeg) {
			...
			this.salarySheetView.trigger('eventMouseover', seg.el[0], seg.event, ev);
		}
	},
	...
	triggerSegMouseout: function(seg, ev) {
		...
		if (this.mousedOverSeg) {
			...
			this.salarySheetView.trigger('eventMouseout', seg.el[0], seg.event, ev);
		}
	},
	...
	computeEventDrop: function(startCell, endCell, event) {
		...
		if (!this.salarySheetView.calendar.isEventRangeAllowed(dropLocation, event)) {
			return null;
		}
	...
	startExternalDrag: function(el, ev, ui) {
		...
		$(document).one('dragstop', function(ev, ui) {
			...
			if (dropLocation) { // element was dropped on a valid date/time cell
				_this.salarySheetView.reportExternalDrop(meta, dropLocation, el, ev, ui);
			}
		});
	...
	computeExternalDrop: function(cell, meta) {
		...
		if (!this.salarySheetView.calendar.isExternalDropRangeAllowed(dropLocation, meta.eventProps)) {
			return null;
		}
	...
	getEventTimeText: function(range, formatStr) {
		...
		if (range.end && this.displayEventEnd) {
			return this.salarySheetView.formatRange(range, formatStr);
		}
		else {
	...
	eventsToNormalRanges: function(events) {
		var calendar = this.salarySheetView.calendar;
		var ranges = [];
		var i, event;
...
var DayGrid = Grid.extend({
	...
	computeColHeadFormat: function() {
		if (this.rowCnt > 1) { // more than one week row. day numbers will be in each cell
			...
			return this.salarySheetView.opt('dayOfMonthFormat'); // "Sat 12/10"
		}
	...
	computeEventTimeFormat: function() {
		return this.salarySheetView.opt('extraSmallTimeFormat'); // like "6p" or "6:30p"
	},
	...
	rangeToSegs: function(range) {
		...
		range = this.salarySheetView.computeDayRange(range); // make whole-day range, considering nextDayThreshold
	...
	renderDrag: function(dropLocation, seg) {
		...
		this.renderHighlight(
			this.salarySheetView.calendar.ensureVisibleEventRange(dropLocation) // needs to be a proper range
		);
		...
		if (seg && !seg.el.closest(this.el).length) {
			...
			opacity = this.salarySheetView.opt('dragOpacity');
...
DayGrid.mixin({
	...
	getMoreLinkText: function(num) {
		var opt = this.salarySheetView.opt('eventLimitText');
...
var TimeGrid = Grid.extend({
	...
	renderBusinessHours: function() {
		var events = this.salarySheetView.calendar.getBusinessHoursEvents();
	...
	// Computes a default event time formatting string if `timeFormat` is not explicitly defined
	computeEventTimeFormat: function() {
		return this.salarySheetView.opt('noMeridiemTimeFormat'); // like "6:30" (no AM/PM)
	},
	...
	computeCellRange: function(cell) {
		...
		var start = this.salarySheetView.calendar.rezoneDate(cell.day).time(time);
	...
	renderSelection: function(range) {
		if (this.salarySheetView.opt('selectHelper')) { // this setting signals that a mock helper event should be rendered
...
TimeGrid.mixin({
	...
	generateSegPositionCss: function(seg) {
		var shouldOverlap = this.salarySheetView.opt('slotEventOverlap');
